...
string filePath = @"frontend\src\components\Canvas.tsx";
string content = File.ReadAllText(filePath);

// 1. Add chatBlocked state
content = content.Replace("const [isTyping, setIsTyping] = useState(false);\n  const chatBottomRef = useRef<HTMLDivElement>(null);", "const [isTyping, setIsTyping] = useState(false);\n  const [chatBlocked, setChatBlocked] = useState(false);\n  const chatBottomRef = useRef<HTMLDivElement>(null);");
...
content = content.Replace("  useEffect(() => {\n    chatBottomRef.current?.scrollIntoView({ behavior: \"smooth\" });\n  }, [chatHistory, isTyping]);", effectToAdd);

// 3. Update the modal condition and UI
string oldModal = @"{activeScreen === ""assistant"" && !user && (
...
string newModal = @"{activeScreen === ""assistant"" && (!user || chatBlocked) && (
            <div className=""flex flex-col items-center justify-center min-h-[480px] p-10 animate-fadeIn text-center gap-6"">
              <div className=""w-20 h-20 rounded-full bg-emerald-50 border-2 border-emerald-200 flex items-center justify-center"">
                <Bot className=""w-9 h-9 text-emerald-700"" />
              </div>
