...
public class ChatToolExecutor
{
    ...
    private async Task<object> GetNearbyPartnersAsync(JsonDocument args, CancellationToken ct)
    {
        ...
        var all = await _partners.GetActiveAsync(ct: ct);
        var nearby = all
            .Where(p => p.Latitude.HasValue && p.Longitude.HasValue
                        && HaversineKm(lat, lon, (double)p.Latitude.Value, (double)p.Longitude.Value) <= radiusKm)
            .Where(p => categoryFilter == null || p.Type.ToString().Equals(categoryFilter, StringComparison.OrdinalIgnoreCase))
            .OrderBy(p => HaversineKm(lat, lon, (double)p.Latitude!.Value, (double)p.Longitude!.Value))
            .Take(10)
            .Select(p => new
            {
                id = p.Id,
                ...
                distanceKm = Math.Round(HaversineKm(lat, lon, (double)p.Latitude!.Value, (double)p.Longitude!.Value), 2),
                address = p.Address
            })
            .ToList();
